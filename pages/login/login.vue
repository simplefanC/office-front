<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="staffNo" type="text" maxlength="12" placeholder="工号" :focus="isFocus"></wInput>
				<wInput v-model="passData" type="password" maxlength="12" placeholder="密码"></wInput>
			</view>
...
export default {
	...
	methods: {
		...
		startLogin(e) {
			...
			let that = this;
			that.ajax(that.url.loginByPass, 'POST', { id: that.staffNo, password: that.passData }, function(resp) {
		...
		login_weixin() {
			...
			uni.login({
				...
				success: function(resp) {
					...
					that.ajax(that.url.login, 'POST', { code: code }, function(resp) {
						//TODO 判断是否绑定
						if (resp.data.msg == '帐户不存在') {
							uni.navigateTo({
								url: '../login/binding?openId=' + resp.data.openId
							});
						} else {
							let permission = resp.data.permission;
							uni.setStorageSync('permission', permission);
