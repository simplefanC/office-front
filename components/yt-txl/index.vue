<template>
	<view class="yt-txl-container">
		<view class="search"><input type="text" @input="inputFunc" class="s-input" placeholder="搜索" /></view>
				...
				<view :id="o.key" v-for="(o, i) in resource" :key="i">
					...
					<view @click="clickFunc(item)" v-for="(item, index) in o.data" class="info" :key="index">
						...
						<view class="item">
							...
							<text class="post">{{ item.deptName }}</text>
						</view>
					</view>
				</view>
			</scroll-view>
		...
		<view class="flag" v-if="index">
			<scroll-view scroll-y="true" :show-scrollbar="false" class="flag-scroll" style="width: 100%;height: 100%;">
				<view @click="toFunc(o.key)" class="flag-key" v-for="(o, i) in resource" :key="i">{{ o.key }}</view>
			</scroll-view>
		</view>
	</view>
</template>
